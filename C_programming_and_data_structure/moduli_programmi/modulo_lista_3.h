...
4) cercare un elemento all'interno della lista per venire a conoscenza della sua posizione
...
4) posList( data, top ) -> pos
                                  pre_condition: (top != NULL) && (data != NULL)
                                  post_condition: (pos = se il dato è presente nella lista,
                                                  pos sarà un intero che riporta la sua posizione
...
6) deletePos( pos, top ) -> flag
                                  pre_condition: (pos >= 0) && (pos < sizeList( top ) - 1 ) && (top != NULL)
...
7) deleteList( top ) -> flag
                                  pre_condition: (top != NULL)
                                  post_condition: (flag = 1 se sizeList( top ) = 0) || (flag = 0 se la lista non è stata
                                                  eliminata)

8) printPos( pos, top ) ->
                                  pre_condition: (pos >= 0) && (pos < sizeList( top )) && (top != NULL)
                                  post_condition: stampa l'elemento di posizione ricercata

9) printList( top ) ->
                                  pre_condition: (top != NULL)
                                  post_condition: stampa gli elementi della lista con la tecnica FILO

10) cloneList( top ) -> cloneTop
                                  pre_condition: (top != NULL)
                                  post_condition: (cloneTop = top)

11) searchList( top, data) -> flag
                                  pre_condition: (pos >= 0) && (pos < sizeList( top )) && (top != NULL)
                                  post_condition: (flag = 1 se l'elemento è presente in lista) || (flag = 0
                                                  se l'elemento non è presente in lista)

12) reverseList( top ) -> topReversed
                                  pre_condition: (top != NULL)
                                  post_condition: (top = <e_1, e_2, e_3, ..., e_n> &&
                                                  topReversed = <e_n, ..., e_3, e_2, e_1> dove "e" è un
                                                  elemento generico)
...
17) deleteNode( node ) -> flag
                                  pre_condition: (node != NULL)
                                  post_condition: (flag = 1 se nodo cencellato) || (flag = 0 se nodo
                                                  non cancellato)
------------------------------------------------------------------------------*/
